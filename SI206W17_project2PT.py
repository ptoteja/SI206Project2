...
def find_urls(s):
	url_list = re.findall(r'\bhttp://\b\w+\.\S{2}[^\s]+|\bhttps://\b\w+\.\S{2}[^\s]+', s)   #Matches beginning http or https, any amount of alphanumerics, the period plus the next two characters following (cannot be whitespace), the end of the url until the space.
	return url_list
...
def get_umsi_data():
	...
	if "umsi_directory_data" in CACHE_DICTION:
		...
		f.close()
	#print(umsi_list[0][:100]) ###################################All values are the same but getting "fail"
	return umsi_list
...
def get_five_tweets(input_string):
	...
	if input_string in CACHE_DICTION:
		...
		search_results = api.search(input_string)       ##################################### Why is it returning 3 tweets instead of 5
...
#	def test_begin_of_list(self):          #Permission from Jackie to omit this test since there is a whitespace error
...
class PartThree(unittest.TestCase):
	def test_get_tweets(self):
		self.assertEqual(len(get_five_tweets("University of Michigan")),5,"Testing that get_five_tweets returns a sequence of 5 things")   #why does this query only return a list of size 4? McGill U returns one of length 5    # uncomment this line
		#self.assertEqual(len(get_five_tweets("McGill University")),5,"Testing that get_five_tweets returns a sequence of 5 things")   #delete this line, not apart of given tests
